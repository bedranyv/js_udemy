...
Examples
songDecoder("WUBWEWUBARETHEWUBCHAMPIONSWUBMYWUBFRIENDWUB")
...
function songDecoder(song) {
    ...
    while (true) {
        ...
        if (foundPos != 0) {
            if (song[foundPos - 1] === ' ' || foundPos + 3 === song.length) {
                songTempPrev = song.substring(0, foundPos);
            } else {
                songTempPrev = song.substring(0, foundPos) + ' ';
            }
        }

        songTemp = songTempPrev + song.substring(foundPos + 3, song.length);
    ...
    while (song[song.length - 1] === ' ') {
        let songTemp = '';
        songTemp = song.substring(0, song.length - 1);
        song = songTemp;
    }
